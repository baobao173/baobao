...
def two_prop_ztest(y1: int, n1: int, y2: int, n2: int) -> dict:
    ...
    z = (p1 - p2) / se
    p = 2 * stats.norm.sf(abs(z))  # 双侧 p 值
...
def main():
    ...
    for _, row in res.iterrows():
        print(
            f"  {row['variable']:<12} χ²={row['chi2']:>12,.0f}  "
    ...
    day = df[df["light"] == "白天"]
    zr = two_prop_ztest(
        int(night["is_severe"].sum()), len(night), int(day["is_severe"].sum()), len(day)
    )
    print(f"  夜间无/弱照明: KSI率={zr['rate1']*100:.2f}% (n={len(night):,})")
    print(f"  白天:          KSI率={zr['rate2']*100:.2f}% (n={len(day):,})")
    print(
        f"  率差={zr['diff']*100:.2f} 个百分点, z={zr['z']:.1f}, "
