...
def write_report(root, config, quality, scores, policies, predictions, business):
    ...
    for row in test.itertuples():
        lines.append(f"| {row.model} | {row.mae:.2f} | {row.wape:.2%} |")
    lines += [
        "",
        f"本次测试 WAPE 最低的是 {test.iloc[0].model}。移动平均相对朴素预测的 WAPE 降幅为 {improvement:.2%}。此为测试表现描述，不用它重新选择 C 策略，也没有据此声称统计显著性。",
        "",
    ...
    for row in base.itertuples():
        lines.append(
            f"| {row.policy} | {row.model} | {row.factor:g} | {row.mean_inventory:.2f} | {row.fill_rate:.2%} | {row.cost:,.0f} |"
        )
    lines += [
        "",
        f"C 相对 A 的测试情景成本{direction} {abs(change):.2%}。选择结果为 {c.model}、k={c.factor:g}。应结合满足率和库存占用解释，而不能只看成本。",
        "",
        f"验证期平均销量为 {business['validation_mean_units']:.2f} 件/商品周，测试期为 {business['test_mean_units']:.2f}，变化 {business['test_vs_validation_growth']:+.1%}。这说明两阶段需求水平不同；只能作为外推风险的描述证据，不能断言它独自导致策略表现变化。验证期适合的安全库存参数没有得到测试期收益保证。",
        "",
        ...
        f"- 测试期只有 {config['test_weeks']} 周，不能覆盖全年泛化表现；没有统计显著性或因果效果声明。",
    ...
    result = (
        f'\n\n- **实际规模**：{quality["raw_rows"]:,} 条原始交易，'
        f'{quality["products"]} 个常销商品，{quality["weeks"]} 个完整周。\n'
        f"- **预测**：移动平均测试 WAPE **{moving.wape:.2%}**，"
        f"较朴素预测降低 **{improvement:.1%}**，本次也略优于岭回归。\n"
        f"- **决策**：基础情景下，验证选择策略 C 的测试成本相对 A **{direction} {abs(change):.1%}**。"
        "这表明验证选择不保证未来收益。\n"
