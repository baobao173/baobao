...
测试期 WAPE 最低的是四周移动平均。四周移动平均相对上周销量基准的 WAPE 降低 20.98%。策略 C 沿用验证期的选择；模型差异尚未做显著性检验。
...
|---|---|---:|---:|---:|---:|
| A | 四周移动平均 | 1 | 184.83 | 84.13% | 133,805 |
| B | 岭回归 | 1 | 177.00 | 81.47% | 144,615 |
| C | 岭回归 | 0.5 | 113.03 | 75.57% | 154,574 |

C 相对 A 的测试情景成本上升 15.52%。验证期选出的组合为岭回归、k=0.5。成本、数量满足率与平均库存共同反映策略表现。

验证期平均销量为 246.42 件/商品周，测试期为 329.24，变化 +33.6%。两个阶段的需求水平不同，固定参数在测试期可能面临需求变化。当前分析未分离需求变化与其他因素各自的影响。
...
- 测试期只有 12 周，尚不能覆盖全年；模型差异和策略收益需要更多时间区间验证。
