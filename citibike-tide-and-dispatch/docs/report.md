...
Citi Bike 2019 年 5—8 月原始记录 8,575,221 条，清洗后 8,564,754 条，覆盖 123 天、2,952 小时、834 个唯一站点。天气取自 Open-Meteo 明确指定的 ERA5 历史再分析。

问题包括时段差异、站点借还失衡、天气与需求的相关关系，以及下一小时系统骑行量预测。

## 方法

1. 排除时间倒置、时长不在 60 秒至 4 小时内、关键站点缺失和异常坐标记录。清洗限制为本次研究月份，按块读取。
...
验证期选择为随机森林。MAE 是平均绝对误差，不能推出“大多数误差都在正负一个 MAE 内”。预测目标是系统总量，没有实施站点级预测。

![预测比较](../output/figures/fig11_forecast_vs_actual.png)

## 站点流量与运营解释

工作日早高峰各站借出减去归还，得到净流量。净借出较大的站点值得先核查库存；净归还较大的站点值得先核查空桩。这个排序既不等于派车数量，也没有确定站点所在地的土地用途。

每日各站正净借出量的和是描述指标，不是调度量下界。早晚车辆可自行回流，站点也可能有足够初始库存。
...
- 缺少库存、容量、调度记录与调度成本，不能声称优化后的实际运营收益。
- 只有四个月、单一城市；修改原模型后沿用同一历史区间，属于修正后的探索性回测，不是新增独立外部验证。
