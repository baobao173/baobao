...
数据背景（用于复试讲解 / README）
----------------------------------
Citi Bike 官方公开数据页：https://citibikenyc.com/system-data
原始文件托管在 Amazon S3：https://s3.amazonaws.com/tripdata/

文件打包方式（2024-02 起调整过）：
  * 2024 年之前：按【年】打包，如 2019-citibike-tripdata.zip（内含各月 CSV）；
  * 2024 年起  ：按【月】打包，如 202401-citibike-tripdata.zip。
本项目使用 2019 年（旧版 15 列字段格式，无疫情扰动），因此走"年度包"分支。

为什么需要多线程分段下载？
--------------------------
S3 服务器在境外，单连接国际带宽往往只有 ~100 KB/s，
818 MB 的文件单线程要下 2 小时以上。HTTP Range 分段 + 多线程并发
通常可提速 3~8 倍，且天然支持断点续传（每个分片独立落盘，重跑自动跳过）。

下载完成后只解压需要的月份，避免整年解压占用磁盘。
"""
...
def download_one_chunk(url: str, start: int, end: int, dest: Path, retries: int = 6) -> bool:
...
def parallel_download(url: str, dest: Path, num_workers: int = 8, chunk_mb: int = 8,
                      retries: int = 6) -> Path:
    ...
    if dest.exists() and dest.stat().st_size == total:
        log.info("目标文件已存在且完整，跳过下载: %s (%.1f MB)", dest.name, total / 1048576)
        return dest

    chunk = chunk_mb * 1024 * 1024
    ranges = [(start, min(start + chunk - 1, total - 1)) for start in range(0, total, chunk)]
    log.info("开始下载 %s (%.1f MB)，共 %d 个分片，%d 线程", dest.name, total / 1048576, len(ranges), num_workers)

    done = 0
    with ThreadPoolExecutor(max_workers=num_workers) as pool:
        futures = {pool.submit(download_one_chunk, url, s, e, dest, retries): (s, e) for s, e in ranges}
...
def extract_months(zip_path: Path, months: list[str], out_dir: Path) -> list[str]:
    ...
    with zipfile.ZipFile(zip_path) as zf:
        ...
        for month in months:
            ...
            hit = [m for m in members if Path(m).name.startswith(f"{year_prefix}{month}-")]
            if not hit:
                log.warning("压缩包中未找到 %s 月的文件（现有条目示例：%s）", month, members[:3])
                continue
            for member in hit:
...
def main() -> int:
    ...
    parser.add_argument("--year", type=int, default=2019, help="数据年份（2024 之前为年度包）")
    parser.add_argument("--months", nargs="+", default=["05", "06", "07", "08"],
                        help="需要解压的月份（两位数字，默认 05 06 07 08）")
    parser.add_argument("--out-dir", type=Path, default=Path("data/raw"), help="原始数据保存目录")
    ...
    parser.add_argument("--retries", type=int, default=6, help="单个分片下载失败的重试次数")
    ...
    parallel_download(url, zip_path, num_workers=args.workers, chunk_mb=args.chunk_mb,
